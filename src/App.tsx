...
export interface EventoActividad {
  ...
  tiempo: string;
  icono: string;
  tipo: "primary" | "tertiary" | "secondary";
}
...
export default function App() {
  ...
  // Integración de LinkedList para Actividad Reciente (Módulo de Romi)
  const [historial, setHistorial] = useState<LinkedList<EventoActividad>>(() => {
    const initList = new LinkedList<EventoActividad>();
    // Inicializamos con eventos de ejemplo simulados (O(1) para prepend)
    initList.prepend({
      id: "3",
      titulo: "Stock Bajo Detectado",
      descripcion: "Aceite de Soja 'Soja Linda' por debajo del mínimo (5 unidades).",
      tiempo: "Ayer",
      icono: "warning",
      tipo: "tertiary"
    });
    initList.prepend({
      id: "2",
      titulo: "Ruta a Asunción Iniciada",
      descripcion: "Vehículo PY-104 en camino con 12 pedidos.",
      tiempo: "Hace 4h",
      icono: "route",
      tipo: "primary"
    });
    initList.prepend({
      id: "1",
      titulo: "Entrada de Mercadería - Código #A102",
      descripcion: "Se recibieron 50 unidades de Yerba Mate 'El Campesino'.",
      tiempo: "Hace 2h",
      icono: "add_box",
      tipo: "primary"
    });
  ...
  const registrarEvento = (titulo: string, descripcion: string, icono: string, tipo: "primary" | "tertiary" | "secondary" = "primary") => {
    ...
    // React trabaja con estados inmutables, por eso no se modifica directamente la lista original.
    // Se realiza una clonación antes de actualizar el estado:
    const nuevaLista = Object.assign(
      Object.create(Object.getPrototypeOf(historial)),
      historial
    );

    nuevaLista.prepend(nuevoEvento);
  ...
  const handleIngresarLote = (e: FormEvent) => {
    ...
    // Registrar el evento en el historial usando LinkedList
    registrarEvento(
      `Entrada de Mercadería - ${productoNombre}`,
      `Se ingresaron ${productoCantidad} unidades a la cola de inventario.`,
      "add_box",
      "primary"
    );
  ...
  const handleRegistrarCliente = (e: FormEvent) => {
    ...
    registrarEvento(
      ...
      `Se agregó a ${nombre} con ID ${clienteId}.`,
      "person_add",
      "primary"
    );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-gutter">
        ...
        <div className="lg:col-span-2 bg-surface-container-lowest rounded-xl border border-outline-variant p-lg shadow-[0_2px_4px_rgba(0,0,0,0.05)]">
          ...
          <div className="space-y-4">
            {historial.toArray().map((evento) => (
              <div key={evento.id} className="flex items-start gap-4 p-4 rounded-lg hover:bg-surface-container-low transition-colors duration-200 border border-transparent hover:border-surface-variant">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${evento.tipo === 'tertiary' ? 'bg-tertiary-container/10 text-tertiary' : 'bg-primary-container/10 text-primary'}`}>
                  <span className="material-symbols-outlined text-md">{evento.icono}</span>
                </div>
                <div className="flex-1">
                  <h4 className="text-body-lg font-body-lg text-on-surface font-medium">{evento.titulo}</h4>
                  <p className="text-body-md font-body-md text-on-surface-variant mt-1">{evento.descripcion}</p>
                </div>
